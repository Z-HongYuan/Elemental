...
void UPocketCapture::SetRenderTargetSize(int32 Width, int32 Height)
{
	if (SurfaceWidth != Width || SurfaceHeight != Height)
	{
		...
		if (DiffuseRT) DiffuseRT->ResizeTarget(SurfaceWidth, SurfaceHeight);
		if (AlphaMaskRT) AlphaMaskRT->ResizeTarget(SurfaceWidth, SurfaceHeight);
		if (EffectsRT) EffectsRT->ResizeTarget(SurfaceWidth, SurfaceHeight);
	}
...
{
	if (DiffuseRT) DiffuseRT->ReleaseResource();
	if (AlphaMaskRT) AlphaMaskRT->ReleaseResource();
	if (EffectsRT) EffectsRT->ReleaseResource();
	//OnReleaseResources();
}

void UPocketCapture::ReclaimResources()
{
	if (DiffuseRT) DiffuseRT->UpdateResource();
	if (AlphaMaskRT) AlphaMaskRT->UpdateResource();
	if (EffectsRT) EffectsRT->UpdateResource();
	//OnReclaimResources();
}
...
TArray<UPrimitiveComponent*> UPocketCapture::GatherPrimitivesForCapture(const TArray<AActor*>& InCaptureActors) const
{
	...
	for (const AActor* CaptureActor : InCaptureActors)
	{
...
UPocketCaptureManager* UPocketCapture::GetThumbnailSystem() const
{
	return CastChecked<UPocketCaptureManager>(GetOuter());
}
